---
title: List Albums
description: Browse albums with pagination, sorting, and genre filtering.
icon: pixelarticons:toke-square
---

## Endpoint

`GET` `/list/albums`

Endpoint reference: [`listAlbums`](/openmusic/endpoints/listAlbums)

## Query Parameters

| Name | Values | Required |
| ---- | ------ | -------- |
| `lang` | locale string | `false` |
| `include` | `artists`, `tracks` | `false` |
| `view` | `minimal` | `false` |
| `view[albums]` | `minimal` | `false` |
| `view[artists]` | `minimal` | `false` |
| `view[songs]` | `minimal` | `false` |
| `limit` | `1-25` | `false` |
| `offset` | `0+` | `false` |
| `sort` | `name`, `-name`, `date_released`, `-date_released`, `date_added`, `-date_added` | `false` |
| `filter[genres]` | comma-separated genres | `false` |
| `limit[artists]` | `1-25` | `false` |
| `limit[tracks]` | `1-25` | `false` |

## Responses

| Status | Content Type | Response |
| ------ | ------------ | -------- |
| `200 OK` | `application/json` | [`ListAlbumsResponse`](/openmusic/list/responses#listalbumsresponse) |
| `400 Bad Request` | `application/json` | [`ErrorResponse`](/openmusic/responses#errorresponse) |
